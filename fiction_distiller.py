#!/usr/bin/env python3
"""
Fiction Distiller — Selective Abridgment for Fiction

Compresses a novel into ~15,000 words via two passes:
  Pass 1: Selectively abridge each chapter with proportional word targets
  Pass 2: Coherence pass — light quality control across the full text

The output preserves the author's original prose, dialogue, and voice.
Scenes that matter stay mostly intact; transitional material is cut or
replaced with minimal bridging narration.

Usage:
    python3 fiction_distiller.py mynovel.epub              # Run all stages
    python3 fiction_distiller.py mynovel.epub --stage 3    # Re-run from stage 3
    python3 fiction_distiller.py mynovel.epub --dry-run    # Stages 1-2 only
...
INSTRUCTIONS_FILE = BASE_DIR / "fiction_distiller_instructions.txt"
COHERENCE_INSTRUCTIONS_FILE = BASE_DIR / "fiction_distiller_coherence_instructions.txt"


def stage3(chapter_files, output_dir, target_words, dry_run=False):
    """Pass 1: Selectively abridge each chapter with proportional word targets."""
    print("\n" + "=" * 60)
    print("STAGE 3: Pass 1 — Abridge Each Chapter")
    print("=" * 60)
    ...
    for fp, content, wc in chapters:
        ...
        chapter_target = max(int(target_words * proportion), 300)
    ...
    for i, (fp, content, word_target) in enumerate(chapter_targets, start=1):
        print(
            f"Abridging chapter {i}/{len(chapter_targets)}: {fp.name} "
            f"(~{word_target} words)…",
            end=" ", flush=True,
        )

        distilled_name = fp.stem + "_fiction_distilled.html"
...
def stage4(results, chapter_files, output_dir):
    """Save fiction-distilled chapters."""
    print("\n" + "=" * 60)
    print("STAGE 4: Save Fiction-Distilled Chapters")
    print("=" * 60)
    ...
    for chapter_path, response_text in results:
        distilled_name = chapter_path.stem + "_fiction_distilled.html"
        distilled_path = output_dir / distilled_name
        response_text = wrap_response_html(response_text, chapter_path, "fiction_distilled")
    ...
    print(f"\nTotal fiction-distilled words: {total_words:,}")

    expected = {fp.stem + "_fiction_distilled.html" for fp in chapter_files}
    actual = {p.name for p in output_dir.iterdir() if p.suffix == ".html"}
    missing = expected - actual
    if missing:
        print(f"\nWARNING: {len(missing)} fiction-distilled file(s) missing.")
    else:
        print(f"All {len(expected)} fiction-distilled chapters present.")
...
def stage5(paths, target_words):
    ...
    print("\n" + "=" * 60)
    print("STAGE 5: Pass 2 — Coherence Check + Final Assembly")
    print("=" * 60)

    from ebooklib import epub

    distilled_dir = paths["fiction_distilled_dir"]
    distilled_files = sorted(distilled_dir.glob("*.html"))
    if not distilled_files:
        print(f"ERROR: No fiction-distilled files found in {distilled_dir}/.")
        sys.exit(1)
    ...
    coherence_cache = output_dir / f"{safe_title}_fiction_coherence_cache.html"
    ...
    if coherence_cache.exists():
        ...
        print("No coherence instructions found. Using raw abridged chapters.")
        ...
        user_message = (
            ...
            f"Here are the chapter-by-chapter abridgments:\n\n"
            f"{combined_html}"
        )
        ...
        if final_html is None:
            print("  Coherence pass failed. Using raw abridged chapters.")
            final_html = combined_html
        else:
    ...
    # Build EPUB
    book = epub.EpubBook()
    book.set_identifier("ebook-fiction-distilled-" + re.sub(r"\W+", "-", book_title.lower()))
    book.set_title(book_title + " (Fiction Distilled)")
...
.bridge { font-style: italic; color: #666; margin: 1.5em 0; }
.chapter-note { background: #f5f5f0; border-left: 3px solid #888; padding: 0.8em 1em; margin: 1em 0 1.5em 0; font-style: italic; color: #555; }
...
"""
    ...
    epub_path = output_dir / f"{safe_title}_fiction_distilled.epub"
    epub.write_epub(str(epub_path), book)
    print(f"\nEPUB saved: {epub_path}")

    html_doc = (
        f"<!DOCTYPE html>\n<html>\n<head>\n<meta charset=\"utf-8\">\n"
        f"<title>{book_title} — Fiction Distilled</title>\n<style>{css_content}</style>\n"
        f"</head>\n<body>\n<h1>{book_title} (Fiction Distilled)</h1>\n"
        f"{final_html}\n</body>\n</html>"
    )
    html_path = output_dir / f"{safe_title}_fiction_distilled.html"
...
def main():
    parser = argparse.ArgumentParser(description="Fiction Distiller — Selective Abridgment for Fiction")
    ...
    parser.add_argument("--target", type=int, default=15000, help="Target word count (default: 15000)")
    ...
    print("=" * 60)
    print("  Fiction Distiller — Selective Abridgment for Fiction")
    print("=" * 60)
    ...
    chapter_files = filter_chapters(chapter_files, paths["selection_file"], "fiction distilling")
    ...
    if args.stage <= 3:
        results = stage3(chapter_files, paths["fiction_distilled_dir"], args.target)

    if args.stage <= 4:
        if results is None:
            results = []
        if results:
            stage4(results, chapter_files, paths["fiction_distilled_dir"])

    if args.stage <= 5:
        stage5(paths, args.target)

    print("\n" + "=" * 60)
    print("  Fiction Distiller pipeline complete!")
    print("=" * 60)
