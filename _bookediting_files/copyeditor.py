...
Sends each chapter of a manuscript to Claude for line-level copyediting.
Returns the full chapter text with corrections applied and a change log.

Usage:
    python3 copyeditor.py mymanuscript.epub              # Run all stages
    python3 copyeditor.py mymanuscript.epub --stage 3    # Re-run from stage 3
    python3 copyeditor.py mymanuscript.epub --dry-run    # Stages 1-2 only
"""
...
from shared import (
    ...
    wrap_response_html, detect_book_metadata, resolve_epub_path,
)

INSTRUCTIONS_FILE = BASE_DIR / "copyeditor_instructions.txt"


def stage3(chapter_files, output_dir, dry_run=False):
    """Send each chapter to the Claude API for copyediting."""
    print("\n" + "=" * 60)
    print("STAGE 3: Send Chapters to Claude API for Copyediting")
    print("=" * 60)
    ...
    # Output is roughly the same size as input (full text + change log)
    estimated_output_tokens = int(total_input_tokens * 1.1)
    ...
    for i, (fp, content) in enumerate(chapters, start=1):
        print(f"Copyediting chapter {i}/{len(chapters)}: {fp.name}…", end=" ", flush=True)

        edited_name = fp.stem + "_copyedited.html"
        edited_path = output_dir / edited_name
        if edited_path.exists():
            print("already processed, skipping.")
            results.append((fp, edited_path.read_text(encoding="utf-8")))
            continue
...
def stage4(results, chapter_files, output_dir):
    """Save copyedited chapters."""
    print("\n" + "=" * 60)
    print("STAGE 4: Save Copyedited Chapters")
    print("=" * 60)
    ...
    for chapter_path, response_text in results:
        edited_name = chapter_path.stem + "_copyedited.html"
        edited_path = output_dir / edited_name
        response_text = wrap_response_html(response_text, chapter_path, "copyedited")
        edited_path.write_text(response_text, encoding="utf-8")
        saved.append(edited_path)
        print(f"  Saved: {edited_name}")

    expected = {fp.stem + "_copyedited.html" for fp in chapter_files}
    actual = {p.name for p in output_dir.iterdir() if p.suffix == ".html"}
    missing = expected - actual

    if missing:
        print(f"\nWARNING: {len(missing)} copyedited chapter(s) missing:")
        for m in sorted(missing):
            print(f"  {m}")
    else:
        print(f"\nAll {len(expected)} copyedited chapters present.")

    return saved


def stage5(paths):
    """Assemble final copyedited EPUB."""
    print("\n" + "=" * 60)
    print("STAGE 5: Assemble Final Copyedited Ebook")
    print("=" * 60)

    from ebooklib import epub

    edited_dir = paths["copyedited_dir"]
    edited_files = sorted(edited_dir.glob("*.html"))
    if not edited_files:
        print(f"ERROR: No copyedited files found in {edited_dir}/.")
        sys.exit(1)

    book_title, book_author = detect_book_metadata(paths["source_dir"])

    book = epub.EpubBook()
    book.set_identifier("ebook-copyedited-" + re.sub(r"\W+", "-", book_title.lower()))
    book.set_title(book_title + " (Copyedited)")
...
.change-log { background: #fff8e1; border-left: 3px solid #f9a825; padding: 0.8em 1em; margin: 1em 0 1.5em 0; font-size: 0.9em; }
.change-log h3 { margin-top: 0.5em; }
ins { background: #e8f5e9; text-decoration: none; }
del { background: #ffebee; text-decoration: line-through; }
"""
    ...
    for i, sf in enumerate(edited_files, start=1):
    ...
    epub_path = output_dir / f"{safe_title}_copyedited.epub"
    epub.write_epub(str(epub_path), book)
    print(f"\nEPUB saved: {epub_path}")

    html_parts = [
        f"<!DOCTYPE html>\n<html>\n<head>\n<meta charset=\"utf-8\">\n"
        f"<title>{book_title} — Copyedited</title>\n<style>{css_content}</style>\n"
        f"</head>\n<body>\n<h1>{book_title} (Copyedited)</h1>\n"
    ]
    for sf in edited_files:
        content = sf.read_text(encoding="utf-8")
        chap_soup = BeautifulSoup(content, "lxml")
        body = chap_soup.find("body")
        if body:
    ...
    html_path = output_dir / f"{safe_title}_copyedited.html"
...
def main():
    parser = argparse.ArgumentParser(description="Manuscript Copyeditor")
    ...
    chapter_files = filter_chapters(chapter_files, paths["selection_file"], "copyediting")
    ...
    if args.stage <= 3:
        results = stage3(chapter_files, paths["copyedited_dir"])

    if args.stage <= 4:
        if results is None:
            results = []
        if results:
            stage4(results, chapter_files, paths["copyedited_dir"])

    if args.stage <= 5:
        stage5(paths)

    print("\n" + "=" * 60)
    print("  Copyeditor complete!")
    print("=" * 60)
