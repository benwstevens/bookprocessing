#!/usr/bin/env python3
"""
Manuscript Editorial Feedback

Sends each chapter of a manuscript to Claude for developmental editorial
feedback — argument strength, pacing, clarity, structure, and reader
engagement. Does not modify the text.

Usage:
    python3 feedback.py mymanuscript.epub              # Run all stages
    python3 feedback.py mymanuscript.epub --stage 3    # Re-run from stage 3
    python3 feedback.py mymanuscript.epub --dry-run    # Stages 1-2 only
"""
...
INSTRUCTIONS_FILE = BASE_DIR / "feedback_instructions.txt"


def stage3(chapter_files, output_dir, dry_run=False):
    """Send each chapter to the Claude API for editorial feedback."""
    print("\n" + "=" * 60)
    print("STAGE 3: Send Chapters to Claude API for Feedback")
    print("=" * 60)
    ...
    # Feedback output is shorter than input
    ...
    for i, (fp, content) in enumerate(chapters, start=1):
        print(f"Getting feedback for chapter {i}/{len(chapters)}: {fp.name}…", end=" ", flush=True)

        feedback_name = fp.stem + "_feedback.html"
        feedback_path = output_dir / feedback_name
        if feedback_path.exists():
            print("already processed, skipping.")
            results.append((fp, feedback_path.read_text(encoding="utf-8")))
            continue
...
def stage4(results, chapter_files, output_dir):
    """Save editorial feedback."""
    print("\n" + "=" * 60)
    print("STAGE 4: Save Editorial Feedback")
    print("=" * 60)
    ...
    for chapter_path, response_text in results:
        feedback_name = chapter_path.stem + "_feedback.html"
        feedback_path = output_dir / feedback_name
        response_text = wrap_response_html(response_text, chapter_path, "feedback")
        feedback_path.write_text(response_text, encoding="utf-8")
        saved.append(feedback_path)
        print(f"  Saved: {feedback_name}")

    expected = {fp.stem + "_feedback.html" for fp in chapter_files}
    actual = {p.name for p in output_dir.iterdir() if p.suffix == ".html"}
    missing = expected - actual

    if missing:
        print(f"\nWARNING: {len(missing)} feedback chapter(s) missing:")
        for m in sorted(missing):
            print(f"  {m}")
    else:
        print(f"\nAll {len(expected)} feedback chapters present.")

    return saved


def stage5(paths):
    """Assemble final feedback EPUB."""
    print("\n" + "=" * 60)
    print("STAGE 5: Assemble Editorial Feedback Ebook")
    print("=" * 60)

    from ebooklib import epub

    feedback_dir = paths["feedback_dir"]
    feedback_files = sorted(feedback_dir.glob("*.html"))
    ...
    book = epub.EpubBook()
    book.set_identifier("ebook-feedback-" + re.sub(r"\W+", "-", book_title.lower()))
    book.set_title(book_title + " (Editorial Feedback)")
...
.strength { background: #e8f5e9; border-left: 3px solid #4caf50; padding: 0.8em 1em; margin: 1em 0; }
.concern { background: #fff3e0; border-left: 3px solid #ff9800; padding: 0.8em 1em; margin: 1em 0; }
"""
    ...
    for i, sf in enumerate(feedback_files, start=1):
    ...
    epub_path = output_dir / f"{safe_title}_feedback.epub"
    epub.write_epub(str(epub_path), book)
    print(f"\nEPUB saved: {epub_path}")

    html_parts = [
        f"<!DOCTYPE html>\n<html>\n<head>\n<meta charset=\"utf-8\">\n"
        f"<title>{book_title} — Editorial Feedback</title>\n<style>{css_content}</style>\n"
        f"</head>\n<body>\n<h1>{book_title} (Editorial Feedback)</h1>\n"
    ]
    for sf in feedback_files:
        content = sf.read_text(encoding="utf-8")
        chap_soup = BeautifulSoup(content, "lxml")
        body = chap_soup.find("body")
        if body:
    ...
    html_path = output_dir / f"{safe_title}_feedback.html"
...
def main():
    parser = argparse.ArgumentParser(description="Manuscript Editorial Feedback")
    ...
    print("=" * 60)
    print("  Manuscript Editorial Feedback")
    print("=" * 60)
    ...
    chapter_files = filter_chapters(chapter_files, paths["selection_file"], "feedback")
    ...
    if args.stage <= 3:
        results = stage3(chapter_files, paths["feedback_dir"])

    if args.stage <= 4:
        if results is None:
            results = []
        if results:
            stage4(results, chapter_files, paths["feedback_dir"])

    if args.stage <= 5:
        stage5(paths)

    print("\n" + "=" * 60)
    print("  Editorial feedback complete!")
    print("=" * 60)
