#!/usr/bin/env python3
"""
Ebook Chapter Abridger Pipeline

Splits an ebook HTML file into chapters, sends each chapter to the Claude API
to be aggressively abridged (cut 40-60% while preserving arguments), then
reassembles the results into a Kindle-compliant EPUB.

Usage:
    python abridger.py                  # Run all stages
    python abridger.py --stage 3        # Re-run from stage 3
    python abridger.py --dry-run        # Stages 1-2 only (no API calls)
"""
...
ABRIDGED_DIR = BASE_DIR / "abridged_chapters"
OUTPUT_DIR = BASE_DIR / "output"
INSTRUCTIONS_FILE = BASE_DIR / "abridger_instructions.txt"
...
def stage3(chapter_files: list[Path], dry_run: bool = False) -> list[tuple[Path, str]]:
    ...
    print("\n" + "=" * 60)
    print("STAGE 3: Send Chapters to Claude API for Abridging")
    print("=" * 60)
    ...
    # Abridging produces output ~50-60% of input size, plus output is more expensive
    estimated_output_tokens = int(total_input_tokens * 0.6)
    ...
    for i, (fp, content) in enumerate(chapters, start=1):
        print(f"Abridging chapter {i}/{len(chapters)}: {fp.name}…", end=" ", flush=True)

        # Check if already processed (resume support)
        abridged_name = fp.stem + "_abridged.html"
        abridged_path = ABRIDGED_DIR / abridged_name
        if abridged_path.exists():
            print("already processed, skipping.")
            results.append((fp, abridged_path.read_text(encoding="utf-8")))
            continue
...
# ===========================================================================
# Stage 4 – Save abridged chapters
# ===========================================================================
def stage4(
    results: list[tuple[Path, str]], chapter_files: list[Path]
) -> list[Path]:
    """Save API responses to abridged_chapters/ and validate completeness."""
    print("\n" + "=" * 60)
    print("STAGE 4: Save Abridged Chapters")
    print("=" * 60)

    ABRIDGED_DIR.mkdir(parents=True, exist_ok=True)
    saved: list[Path] = []

    for chapter_path, response_text in results:
        abridged_name = chapter_path.stem + "_abridged.html"
        abridged_path = ABRIDGED_DIR / abridged_name
        ...
        abridged_path.write_text(response_text, encoding="utf-8")
        saved.append(abridged_path)
        print(f"  Saved: {abridged_name}")

    # Validate completeness
    expected = {fp.stem + "_abridged.html" for fp in chapter_files}
    actual = {p.name for p in ABRIDGED_DIR.iterdir() if p.suffix == ".html"}
    missing = expected - actual

    if missing:
        print(f"\nWARNING: {len(missing)} abridged chapter(s) missing:")
        for m in sorted(missing):
            print(f"  {m}")
        print("Re-run with --stage 3 to process missing chapters.")
    else:
        print(f"\nAll {len(expected)} abridged chapters present.")
...
def stage5(chapter_files: list[Path]) -> Path:
    """Assemble abridged chapters into an EPUB file."""
    print("\n" + "=" * 60)
    print("STAGE 5: Assemble Final Abridged Ebook")
    print("=" * 60)

    from ebooklib import epub

    # Collect abridged files in order
    abridged_files = sorted(ABRIDGED_DIR.glob("*.html"))
    if not abridged_files:
        print("ERROR: No abridged files found in abridged_chapters/.")
        sys.exit(1)
    ...
    # Create EPUB
    book = epub.EpubBook()
    book.set_identifier("ebook-abridged-" + re.sub(r"\W+", "-", book_title.lower()))
    book.set_title(book_title + " (Abridged)")
...
"""
    ...
    for i, sf in enumerate(abridged_files, start=1):
    ...
    epub_path = OUTPUT_DIR / f"{safe_title}_abridged.epub"
    ...
    html_parts = [
        "<!DOCTYPE html>\n<html>\n<head>\n"
        '<meta charset="utf-8">\n'
        f"<title>{book_title} — Abridged</title>\n"
        f"<style>{css_content}</style>\n"
        "</head>\n<body>\n"
        f"<h1>{book_title} (Abridged)</h1>\n"
    ]
    for sf in abridged_files:
        content = sf.read_text(encoding="utf-8")
        chap_soup = BeautifulSoup(content, "lxml")
        body = chap_soup.find("body")
        if body:
    ...
    html_path = OUTPUT_DIR / f"{safe_title}_abridged.html"
...
def filter_chapters(chapter_files: list[Path]) -> list[Path]:
    ...
    print(f"\nSelected {len(selected)} chapters for abridging:")
    for f in selected:
        print(f"  {f.name}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Ebook Chapter Abridger Pipeline"
    )
    parser.add_argument(
        "--stage",
        type=int,
    ...
    print("=" * 60)
    print("  Ebook Chapter Abridger Pipeline")
    print("=" * 60)
    if dry_run:
    ...
    if dry_run:
        ...
        print(f"Selected {len(chapter_files)} chapters for abridging.")
    ...
    if start_stage <= 4:
        ...
        if results:
            ...
            print("\nStage 4: No new results to save. Validating existing files…")
            expected = {fp.stem + "_abridged.html" for fp in chapter_files}
            actual = {
                p.name
                for p in ABRIDGED_DIR.iterdir()
                if p.suffix == ".html"
            } if ABRIDGED_DIR.exists() else set()
            missing = expected - actual
            if missing:
                print(f"  WARNING: {len(missing)} abridged file(s) missing:")
                for m in sorted(missing):
                    print(f"    {m}")
            else:
                print(f"  All {len(expected)} abridged chapters present.")
    ...
    print("\n" + "=" * 60)
    print("  Abridger pipeline complete!")
    print("=" * 60)
